...
def prune_local_score_all(this_score, parent_set, child_dkt):
    for other_parent_set, other_parent_set_score in child_dkt.items():
        print(other_parent_set,parent_set)
        if other_parent_set < parent_set:            
            return True
    return False
    ...
    def bdeu_scores(self,palim=None,pruning=True,alpha=None):
        ...
        for pasize in range(1,palim+1):
            for family in combinations(self._variables,pasize): 
                score_component = self.bdeu_score_component(family,alpha)[0]
        ...
        for vars in combinations(self._variables,palim+1):
            score_component = self.bdeu_score_component(vars,alpha)[0]
